...
class icms_core_Logger extends \Monolog\Logger
{
	...
	static public function &instance()
	{
		...
		if (!isset($instance)) {
			...
			$instance->getExceptionsHandler();
			$instance->getDebugBar();
		}
		return $instance;
	}

	/**
	 * Gets exceptions handler
	 *
	 * @return \Whoops\Run
	 */
	private function getExceptionsHandler()
	{
		static $handler = null;
		if ($handler === null) {
	...
	/**
	 * Returns debugbar instance
	 *
	 * @return \DebugBar\StandardDebugBar
	 */
	private function getDebugBar()
	{
		static $debugbar = null;
		if ($debugbar === null) {
			$debugbar = new \DebugBar\StandardDebugBar();
			$debugbar->addCollector(
				new DebugBar\Bridge\MonologCollector($this)
			);
			$debugbar->addCollector(
				new \DebugBar\DataCollector\PDO\PDOCollector(icms::$db)
			);
			$debugbar->addCollector(
			...
			$debugbar->addCollector(
				new DebugBar\DataCollector\PhpInfoCollector()
			);
			$debugbar->addCollector(
				new DebugBar\DataCollector\ExceptionsCollector()
			);
			$debugbar->addCollector(
	...
	{
		$this->getDebugBar()['time']->startMeasure($name);
	}
	...
	{
		$this->getDebugBar()['time']->stopMeasure($name);
	}
	...
	public function dump($mode = '')
	{
		...
		return $render->renderHead() . $render->render();
	}
