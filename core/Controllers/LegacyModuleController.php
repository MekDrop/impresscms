...
class LegacyModuleController {
	...
	public function setup($config = array())
	{
		...
		if (isset($config['isAdminSide']) && $config['isAdminSide'] === true) {
			...
			$GLOBALS['icmsAdminTpl'] = &$this->theme->template;
		} else {
			$GLOBALS['icmsTpl'] = &$this->theme->template;
		}
		$GLOBALS['icmsTheme'] = &$this->theme;
	...
	/**
	 * Does some thing as old icms footer.php code did
	 */
	protected function execOldFooterCode()
	{
